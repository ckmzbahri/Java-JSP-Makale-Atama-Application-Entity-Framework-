...
<meta charset="utf-9">
    <form class="modal-content animate" action="makaleguncelliyom.jsp" method="post">
...
islem.idaldim(verim);
 if (sonuc=true)
 {
    ...
    while(mkl.next()){ %>
           ...
           <br>
           <input type="text"  name="baslik" required value="<%=mkl.getString("baslik") %>">
           <br><br>
  ...
  while(tr.next()) { %>
    <option value="<%=tr.getString("tur") %>" ><%=tr.getString("tur") %></option>
     ...
     <%} %>
           ...
           <br>
           <textarea id="icerik" name="icerik" rows="4" cols="50" style="width: 700px; height: 700px;" required="required"><%=mkl.getString("icerik") %></textarea>
           <br><br>
