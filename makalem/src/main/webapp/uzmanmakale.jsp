...
<th>DURUM</th>
  ...
  <%
  boolean s=false;
    s=islem.VeriAldım(veri);

    if (s=true)
    {
  	ResultSet rs = islem.selecData();
  	while (rs.next()) {
  %>
     ...
     <td><%=rs.getString("baslik") %></td>
      ...
      <td><%=rs.getString("durum") %></td>	
       ...
       <td><input type ="text" name="id" disabled value= "<%=rs.getString("id")%>"></input></td>
        ...
        {%>
        <td><a href='makalegericevir.jsp?id=<%=rs.getInt("id")%>'>Geri Çevir<span class="glyphicon glyphicon-edit" style="font-size:24px; color:green;"></span></a></td>
         <td><a href='makalekabul.jsp?id=<%=rs.getInt("id")%>'>Kabul Et<span class="glyphicon glyphicon-edit" style="font-size:24px; color:green;"></span></a></td>
         <td>-</td>
     
           <% 
         }%>
          <% 
          String du=rs.getString("rapor");
         if (rs.getString("durum").equals("atandi"))
        {%>
        <%if(du==null){ %>
        <td>-</td>
         <td>-</td>
      <td><a href='raporyaz.jsp?id=<%=rs.getInt("id")%>'>RAPOR YAZ<span class="glyphicon glyphicon-edit" style="font-size:24px; color:green;"></span></a></td>
                <% }
        else
        {
        	%>
