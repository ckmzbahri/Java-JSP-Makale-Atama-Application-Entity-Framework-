...
<meta charset="utf-9">
    <form class="modal-content animate" action="uzmanguncelleme.jsp" method="post">
    ...
    <jsp:useBean id="veri" class="kategori.getset"></jsp:useBean>
        ...
        <jsp:useBean id="verim" class="uzman.getset"></jsp:useBean>
...
islem.idaldim(verim);
 if (sonuc=true)
 {
    ...
    ResultSet  r= islem.select(verim);
    while(r.next()){ %>
           <label for="uname"><b>AD</b></label>
           <br>
           <input type="text"  name="ad" required value="<%=r.getString("ad") %>">
           <br><br>
           <label for="uname"><b>SOYAD</b></label>
           <br>
           <input type="text"  name="soyad" required value="<%=r.getString("soyad") %>">
           <br><br>
  ...
  while(tr.next()) { %>
    <option value="<%=tr.getString("tur") %>" ><%=tr.getString("tur") %></option>
     ...
     <%} %>
           ...
           <br>
          <input type="text"  name="kullanici" required value="<%=r.getString("kullanici") %>">
           <br><br>

         <label for="uname"><b>MAİL</b></label>
           <br>
          <input name="mail" type="text" class="validate[required,custom[]] feedback-input" required= "required" id="mail" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$"title="E-mail formatında giriniz." value="<%=r.getString("mail") %>" />
           <br><br>
